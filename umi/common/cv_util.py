...
def get_gripper_width(tag_dict, left_id, right_id, nominal_z=0.21, z_tolerance=0.05):
...
def get_s10_gripper_canonical_polygon():
    """
    Gripper mask polygon for Galaxy S10 ultrawide (1920x1080) mounted on UMI gripper.
    Coordinates measured visually from actual S10 footage.
    SLAM용: 그리퍼 팔 + 하단 태그 영역 전체를 가림.

    Pixel coords reference (1920x1080):
      - Left arm:  x=255..644  y=704..1000
      - Right arm: x=1320..1652  y=705..994
      - Bottom center strip: tag/finger area between arms
    """
    ...
    # bottom-center strip (finger/tag area between arms)
    bottom_pts = [
        [128, 1080],
        [753, 533],
        [1154, 533],
    ...
    coords = []
    for pts in [left_pts, right_pts, bottom_pts]:
        coords.append(pixel_coords_to_canonical(pts, resolution))
    return coords


def draw_s10_slam_mask(img, color=(0, 0, 0)):
    """SLAM용: 그리퍼 팔 + 하단 태그 영역 전체를 가림."""
    for coords in get_s10_gripper_canonical_polygon():
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        cv2.fillPoly(img, [pts], color=color)
    return img
...
def get_s21_gripper_canonical_polygon():
    """
    Gripper mask polygon for Galaxy S21 (1920x1080) mounted on UMI gripper.
    S21 실제 영상을 촬영한 뒤 python droid_slam_s21/visualize_slam_mask.py 로 확인하고
    실제 그리퍼 팔 위치에 맞게 아래 좌표를 수정하세요.
    """
    resolution = [1080, 1920]  # [height, width]
...
def draw_s21_slam_mask(img, color=(0, 0, 0)):
    """SLAM용 S21 그리퍼 마스크. 실제 S21 영상으로 좌표 확인 후 사용하세요."""
    for coords in get_s21_gripper_canonical_polygon():
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        cv2.fillPoly(img, [pts], color=color)
    return img
...
def get_s21_training_polygon():
    """
    S21 학습/eval 이미지용 마스크 좌표.
    하단 그리퍼 영역(하단 스트립)만 가림. S10과 동일한 좌표를 사용.
    """
    resolution = [1080, 1920]  # [height, width]
    ...
    return [pixel_coords_to_canonical(bottom_pts, resolution)]


def draw_s21_training_mask(img, color=(0, 0, 0)):
    """학습/eval 이미지용: 하단 그리퍼 영역을 가림."""
    img = np.ascontiguousarray(img)
    for coords in get_s21_training_polygon():
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        cv2.fillPoly(img, [pts], color=color)
    return img
...
def get_s10_training_polygon():
    """
    S10 학습/eval 이미지용 마스크 좌표.
    실제 S10 영상(1920x1080)에서 직접 측정한 하단 그리퍼 영역.
    """
    resolution = [1080, 1920]  # [height, width]
...
def draw_s10_training_mask(img, color=(0, 0, 0)):
    """학습/eval 이미지용: 하단 그리퍼 영역을 가림."""
    img = np.ascontiguousarray(img)
    for coords in get_s10_training_polygon():
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        cv2.fillPoly(img, [pts], color=color)
    return img
