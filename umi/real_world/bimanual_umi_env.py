...
class BimanualUmiEnv:
    def __init__(self, 
        ...
        # Fallback: if no by-id devices found (e.g. Iriun virtual webcam),
        ...
        # Changed the type hint for the grippers list
        grippers: List[DynamixelController] = list()
        ...
        for gc in grippers_config:
            # Updated the constructor call to use DynamixelController parameters
            this_gripper = DynamixelController(
                shm_manager=shm_manager,
                device_name=gc['device_name'],
                baudrate=gc['baudrate'],
                dxl_ids=gc['dxl_ids'],
                gripper_max_width_mm=gc['gripper_max_width_mm'],
                motor_positions_open=gc['motor_positions_open'],
                motor_positions_closed=gc['motor_positions_closed'],
    ...
    def get_obs(self) -> dict:
        ...
        for robot_idx, last_gripper_data in enumerate(last_grippers_data):
            ...
            gripper_interpolator = get_interp1d(
                ...
                x=last_gripper_data['gripper_position_mm'][...,None] # Changed key from gripper_position to gripper_position_mm
            )
            gripper_obs = {
        ...
        if self.obs_accumulator is not None:
            ...
            for robot_idx, last_gripper_data in enumerate(last_grippers_data):
                self.obs_accumulator.put(
                    data={
                        # Added Dynamixel-specific obs keys
                        f'robot{robot_idx}_gripper_width': last_gripper_data['gripper_position_mm'][...,None],
                        f'robot{robot_idx}_gripper_velocity': last_gripper_data['gripper_velocity_mm_s'][...,None],
                        f'robot{robot_idx}_gripper_force': last_gripper_data['gripper_force'][...,None]
                    },
