...
> **경우 D 주의**: 12.8 설치 시 `/usr/local/cuda` 심볼릭 링크가 12.8을 가리키도록 자동 변경될 수 있다. 이 링크를 참조하는 기존 작업이 있다면 영향을 받을 수 있으므로, 설치 전후 아래 명령으로 링크 상태를 확인한다.
> ```bash
> ls -la /usr/local/cuda
> ```
> 링크가 바뀐 경우 아래 명령으로 원래 버전으로 되돌릴 수 있다 (예: 12.1이었던 경우):
> ```bash
