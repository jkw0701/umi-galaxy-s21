...
from umi.common.pose_util import pose_to_mat, mat_to_pose, pose10d_to_mat, mat_to_pose10d
...
class TemporalEnsembler:
    ...
    def get_next_actions(self, shift_steps: int, output_steps: int) -> np.ndarray:
        """
        shift_steps: 실제 시간이 흐른 만큼 버퍼를 미는 양 (예: steps_per_inference)
        output_steps: 로봇에게 보내기 위해 꺼내는 양 (예: steps_per_inference + latency_margin)
        """
        # 1. Output 만큼 꺼내기 (여유분 포함해서 길게 꺼냄)
        # Count가 0인 부분은 1로 나눠서 NaN 방지
        current_count = np.maximum(self.action_count[:output_steps], 1.0)
        averaged_action = self.action_sum[:output_steps] / current_count
        
        # 2. Shift는 정확히 주기(steps_per_inference) 만큼만!
        self.action_sum[:-shift_steps] = self.action_sum[shift_steps:]
        self.action_count[:-shift_steps] = self.action_count[shift_steps:]
        
        # 3. 뒤쪽 빈 공간 초기화
        self.action_sum[-shift_steps:] = 0.0
        self.action_count[-shift_steps:] = 0.0
        
        return averaged_action


DISPLAY = False
VIS_INPUT = False  # 모델 입력 이미지 시각화 (DISPLAY와 독립)
...
@click.option('--mask_mode', type=click.Choice(['predefined', 's21', 'none']), default='predefined',
              help='Camera image masking: predefined=GoPro gripper polygon, s21=S21 gripper polygon, none=no mask')
...
def main(input, output,
    ...
    with SharedMemoryManager() as shm_manager:
        with KeystrokeCounter() as key_counter, \
            ...
            origin_save_path = os.path.join(ROOT_DIR, 'example', 'origin_pose.json')
            if os.path.exists(origin_save_path):
                with open(origin_save_path, 'r') as f:
                    saved = json.load(f)
                origin_pose = np.array(saved['origin_pose'])
                origin_gripper_pos = np.array(saved['origin_gripper_pos'])
                print(f"📂 Origin loaded from {origin_save_path}")
            else:
                origin_pose = np.array([0.3024, 0.333, 0.3628, -1.9036, -0.0815, -0.0866])
                origin_gripper_pos = np.array([87.0])
                print(f"Origin Hardcoded (no saved file found)")

            with np.printoptions(precision=3, suppress=True):
                print(f"   Pos: {origin_pose[:3]}")
                print(f"   Rot: {origin_pose[3:]}")
                print(f"   Gripper: {origin_gripper_pos}")
            ...
            while True:
                ...
                while True:
                    ...
                    for key_stroke in press_events:
                        ...
                        # initial pose setting
                        if key_stroke == KeyCode(char='b'):
                            print("\n📍 Marking CURRENT Position as NEW ORIGIN...")
                            
                            # 현재 로봇 상태 받아오기
                            current_state = env.get_robot_state()
                            current_gripper = env.get_gripper_state()
                            
                            # Origin 변수 업데이트
                            origin_pose = current_state['ActualTCPPose'].copy()
                            origin_gripper_pos = current_gripper['gripper_position_mm'].copy()
                            
                            # 파일에 저장
                            save_data = {
                                'origin_pose': origin_pose.tolist(),
                                'origin_gripper_pos': origin_gripper_pos.tolist() if isinstance(origin_gripper_pos, np.ndarray) else [origin_gripper_pos]
                            }
                            with open(origin_save_path, 'w') as f:
                                json.dump(save_data, f, indent=2)

                            # 값 출력
                            with np.printoptions(precision=4, suppress=True):
                                print(f"✅ New Origin Saved to {origin_save_path}")
                                print(f"   Pos: {origin_pose[:3]}")
                                print(f"   Rot: {origin_pose[3:]}")
                                print(f"   Gripper: {origin_gripper_pos:.1f}")
                            print("-" * 30)
                        if key_stroke == KeyCode(char='v'):
                            print("\n🚀 Moving to Origin (Start Pose)...")
                            duration = 5.0 # 3초 동안 이동
                            
                            # 현재 위치에서 원점까지 보간(Interpolation)하여 이동
                            # (갑자기 튀는 것을 방지하기 위해 쪼개서 명령을 보냅니다)
                            
                            start_pose_interp = target_pose.copy()
                            start_gripper_interp = target_gripper_pos_mm.copy()
                            
                            steps = int(duration * frequency) # 총 스텝 수 계산
                            
                            r_start = st.Rotation.from_rotvec(start_pose_interp[3:])
                            r_end = st.Rotation.from_rotvec(origin_pose[3:])
                            slerp = st.Slerp([0.0, 1.0], st.Rotation.concatenate([r_start, r_end]))
                            ...
                            for i in range(steps):
                                ...
                                # 위치/그리퍼: 선형 보간
                                interp_pos = (1 - alpha) * start_pose_interp[:3] + alpha * origin_pose[:3]
                                interp_gripper = (1 - alpha) * start_gripper_interp + alpha * origin_gripper_pos
                                # 회전: SLERP (최단 경로 보간)
                                interp_rot = slerp(alpha).as_rotvec()
                                
                                # 액션 생성
                                safe_action = np.zeros(7)
                                safe_action[:3] = interp_pos
                                safe_action[3:6] = interp_rot
                                safe_action[6] = interp_gripper
                                
                                # 로봇에 명령 전송
                                env.exec_actions(actions=np.array([safe_action]), timestamps=np.array([time.time() + dt]))
                                
                                # 주기 맞추기
                                time.sleep(dt)
                            
                            # 이동 완료 후, 타겟 변수들을 원점 값으로 업데이트
                            # (그래야 다음 키보드 입력이 원점 기준에서 시작됨)
                            target_pose = origin_pose.copy()
                            target_gripper_pos_mm = origin_gripper_pos.copy()
                            
                            print("✅ Arrived at Origin.\n")
                            continue # 이번 루프는 여기서 끝내고 다시 키 입력 대기
                ...
                try:
                    ...
                    # reset ensembler
                    # NOTE: ensemble in raw model space (pos3 + rot6d + grip = 10D per robot)
                    # to avoid axis-angle flip averaging bug near pi boundary.
                    raw_action_dim = int(cfg.task.shape_meta.action.shape[0])
                    ensembler = TemporalEnsembler(max_steps=50, action_dim=raw_action_dim)

                    # first step flag
                    is_first_policy_step = True
                    ...
                    while True:
                        ...
                        with torch.no_grad():
                            ...
                            if VIS_INPUT:
                                vis_frames = []
                                for cam_key in sorted(k for k in obs_dict_np if k.endswith('_rgb')):
                                    # obs_dict_np: (T, C, H, W) float32 [0,1]
                                    img_chw = obs_dict_np[cam_key][-1]
                                    img_hwc = np.moveaxis(img_chw, 0, -1)
                                    img_u8  = (img_hwc * 255).clip(0, 255).astype(np.uint8)
                                    img_bgr = np.ascontiguousarray(img_u8[..., ::-1])
                                    cv2.putText(img_bgr, cam_key, (4, 16),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
                                    vis_frames.append(img_bgr)
                                if vis_frames:
                                    cv2.imshow('Model Input', np.concatenate(vis_frames, axis=1))
                                    cv2.waitKey(1)
                            # ─────────────────────────────────────────────────────
                            ...
                            # =================================================================
                            # FIX v2: Ensemble in ABSOLUTE rot6d space.
                            #   - axis-angle 공간에서 평균 ⇒ π 경계 flip 버그 (identity 명령 튐)
                            #   - relative 공간에서 평균 ⇒ 각 chunk의 base pose가 달라서 목표 흔들림 (끊김)
                            # 해결: 상대→절대 변환은 rot6d를 유지한 채 수행하고, 그 뒤에 ensembling
                            # =================================================================

                            # 1. Relative rot6d → Absolute rot6d (single-robot assumption)
                            action_pose10d = raw_action[..., :9]              # (T, 9)
                            action_grip    = raw_action[..., 9:10]            # (T, 1)
                            action_mat_rel = pose10d_to_mat(action_pose10d)   # (T, 4, 4)

                            base_pose = np.concatenate([
                                obs['robot0_eef_pos'][-1],
                                obs['robot0_eef_rot_axis_angle'][-1]
                            ], axis=-1)
                            base_mat = pose_to_mat(base_pose)                 # (4, 4)

                            if action_pose_repr == 'relative':
                                action_mat_abs = base_mat @ action_mat_rel
                            else:
                                action_mat_abs = action_mat_rel               # already absolute
                            abs_pose10d = mat_to_pose10d(action_mat_abs)      # (T, 9) abs pos+rot6d
                            abs_action = np.concatenate([abs_pose10d, action_grip], axis=-1)  # (T,10)

                            # 2. Ensemble absolute 10D (pos3 + rot6d6 + grip1)
                            ensembler.update(abs_action)
                            final_abs_action = ensembler.get_next_actions(
                                shift_steps=steps_per_inference,
                                output_steps=16
                            )

                            # 3. Absolute rot6d → axis-angle (for robot controller)
                            final_pose10d = final_abs_action[..., :9]
                            final_grip    = final_abs_action[..., 9:10] * 1000.0  # m → mm
                            final_mat     = pose10d_to_mat(final_pose10d)
                            final_pose6   = mat_to_pose(final_mat)            # (T, 6) abs pos+axis-angle
                            action = np.concatenate([final_pose6, final_grip], axis=-1)  # (T, 7)
                        ...
                        if is_first_policy_step:
                            # First inference: Do not discard any frames due to latency.
                            # Realign the timestamps relative to the 'current_time' to ensure a smooth, immediate start.
                            action_timestamps = (np.arange(len(action), dtype=np.float64)) * dt + curr_time + action_exec_latency
                            is_new = np.ones(len(action), dtype=bool)  # All actions are new on first step
                            is_first_policy_step = False
                        else:
                        ...
                        step_log = {
                            ...
                            # 1. total Chunk (Raw & Processed)
                            'raw_action_chunk': raw_action.copy(),  # raw actgion
                            'processed_action_chunk': action.copy(), # procssed action
                            
                            # 2. find filtering
                            'action_timestamps': action_timestamps.copy(),
                            'is_new_mask': is_new.copy(),
                            
                            # 3. robot action commain (Executed)
                            'executed_action': this_target_poses.copy() if len(this_target_poses) > 0 else None,
                            'executed_timestamps': action_timestamps.copy() if len(action_timestamps) > 0 else None,
                            
                            # 4. current robot state (Result - in fact this data is previous state)
                            'robot_eef_pos': obs['robot0_eef_pos'][-1].copy(),
                            'robot_eef_rot_axis_angle': obs['robot0_eef_rot_axis_angle'][-1].copy(),
                            'robot_gripper_width': obs['robot0_gripper_width'][-1].copy(),
                            
                            # 5. saving observation image 
                            'obs_image': obs['camera0_rgb'][-1].copy(),
                        }
                        episode_debug_log.append(step_log)
                        # =================================================================
                        ...
                        if stop_episode:
                            ...
                            # saving logs
                            ep_idx = env.replay_buffer.n_episodes - 1
                            log_path = os.path.join(output, 'videos', str(ep_idx), 'debug_log.pkl')
                        
                            os.makedirs(os.path.dirname(log_path), exist_ok=True)
                            
                            print(f"💾 Saving debug log to: {log_path}")
                            with open(log_path, 'wb') as f:
                                pickle.dump(episode_debug_log, f)
                            # saving logs
                            break
