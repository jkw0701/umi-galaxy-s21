...
`03_batch_slam.py`는 프레임 추출 시 그리퍼 영역을 검은색으로 덮어씌운 뒤 DROID-SLAM에 입력한다.

**이유**: DROID-SLAM은 프레임 간 광학 흐름(optical flow)을 추적해 카메라 포즈를 추정한다. 그리퍼가 카메라 하단에 고정 장착되어 있어 항상 시야에 잡히는데, 그리퍼가 열리고 닫히는 움직임을 SLAM이 카메라 자체의 움직임으로 잘못 해석하면 포즈 추정이 오염된다. 마스킹으로 그리퍼 영역을 제거해 이 문제를 방지한다.

![그리퍼 마스킹 비교](docs/images/gripper_mask_comparison.png)

왼쪽이 원본 프레임, 오른쪽이 SLAM에 실제로 입력되는 마스킹된 프레임이다. 하단 그리퍼 손가락 부분이 검은색으로 제거된 것을 확인할 수 있다.
