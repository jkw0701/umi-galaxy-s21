...
초기 파라미터(filter_thresh=2.4, keyframe_thresh=4.0, warmup=8) 상태에서 로봇이 공을 잡고 이동하는 90~150프레임 구간에서 20~30개 에피소드의 Z축이 크게 튀었습니다.

- `filter_thresh` 2.4 → 3.4: 움직임이 많은 구간의 저신뢰도 대응점 제거 강화
- `keyframe_thresh` 4.0 → 1.0: 빠른 움직임 구간을 키프레임으로 더 촘촘하게 커버

→ 90~150프레임 구간 불안정 해소. 그러나 keyframe_thresh를 1.0으로 낮추자 초반부터 키프레임이 과도하게 생성되어 **0~50프레임 구간에서 60개 에피소드가 새롭게 불안정**해지는 문제 발생.
...
warmup=8이면 BA가 수렴하기에 제약 조건이 너무 부족한 상태로 초기 자세 추정이 잘못된 해로 수렴하고, 이 오류가 이후 전체 프레임에 누적·전파됩니다. `warmup`을 8 → 26으로 높여 초기 그래프가 충분한 키프레임을 확보하고 나서 추적을 시작하게 하자 **120개 에피소드 전 구간에서 불안정 에피소드가 0개**로 완전히 해소됐습니다.
...
> **핵심 교훈**: 90~150프레임 구간의 문제는 filter/keyframe 파라미터 조정으로 해결 가능했으나, 초반 구간의 근본적인 불안정은 SLAM 초기화 메커니즘인 warmup 부족에서 비롯된 것이었습니다. warmup을 26으로 높이는 것이 결정적인 해결책이었습니다.
