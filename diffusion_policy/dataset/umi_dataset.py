...
class UmiDataset(BaseDataset):
    def __init__(self,
        ...
        # ── 데이터 파일 구조 확인 ──────────────────────────────────────────────
        print(f"\n[Dataset] 로드 완료: {dataset_path}")
        print(f"  에피소드 수: {replay_buffer.n_episodes}, 총 스텝 수: {replay_buffer.n_steps}")
        print(f"  Zarr 키 목록:")
        for key in sorted(replay_buffer.keys()):
            arr = replay_buffer[key]
            print(f"    {key:40s}  shape={arr.shape}  dtype={arr.dtype}")
        # ─────────────────────────────────────────────────────────────────────
        ...
        # ── shape_meta 파싱 결과 확인 ─────────────────────────────────────────
        print(f"\n[Dataset] shape_meta 파싱 결과")
        print(f"  RGB 키:      {rgb_keys}")
        print(f"  Low-dim 키:  {lowdim_keys}")
        print(f"  로봇 수:     {self.num_robot}")
        print(f"  train 에피소드: {train_mask.sum()}  /  val 에피소드: {val_mask.sum()}")
        print(f"  horizon (key→steps): { {k: key_horizon[k] for k in list(key_horizon)[:6]} }")
        # ─────────────────────────────────────────────────────────────────────
