    ...
    def run(self):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if accelerator.is_main_process:
                    print(f"\n{'='*60}")
                    print(f"[Epoch {self.epoch}]  global_step={self.global_step}")
                    print(f"  train_loss        : {step_log.get('train_loss', float('nan')):.6f}")
                    print(f"  lr                : {step_log.get('lr', float('nan')):.2e}")
                    # 액션 MSE (diffusion sampling 결과)
                    for tag in ('train', 'val'):
                        mse = step_log.get(f'{tag}_action_mse_error')
                        if mse is not None:
                            pos  = step_log.get(f'{tag}_action_mse_error_pos',  '?')
                            rot  = step_log.get(f'{tag}_action_mse_error_rot',  '?')
                            grip = step_log.get(f'{tag}_action_mse_error_width','?')
                            def _f(v): return f"{float(v):.6f}" if v != '?' else '?'
                            print(f"  {tag}_action_mse   : {_f(mse)}"
                                  f"  (pos={_f(pos)}, rot={_f(rot)}, grip={_f(grip)})")
                    print(f"{'='*60}\n")
                # ─────────────────────────────────────────────────────────────────
