...
def main(robot_hostname, frequency):
    ...
    with SharedMemoryManager() as shm_manager:
        ...
        with FrankaInterpolationController(
            ...
            print('Ready! Robot will move in Z-axis automatically.')
            print('⚠️ KEEP HANDS NEAR E-STOP! (비상 정지 대기)')
            time.sleep(1.0) # 준비 시간

            # 현재 위치 저장 (기준점)
            state = controller.get_state()
            start_pose = state['ActualTCPPose'].copy() # [x, y, z, rx, ry, rz]
            ...
            while True:
                ...
                # [핵심] 사인파로 Z축 목표 위치 계산
                # sin(시간) * 5cm
                z_offset = np.sin(t_elapsed * 2 * np.pi / period) * amplitude
                
                target_pose = start_pose.copy()
                target_pose[2] += z_offset # Z축에만 변화를 줌
