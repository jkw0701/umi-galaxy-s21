...
> **[Deprecated]** 이 가이드는 `umi` + `droid` 분리 환경 기반으로 작성되었다.  
> 현재 파이프라인은 `umi_full` 통합 환경을 사용하므로, 이 가이드대로 설치해도 정상 동작하지 않는다.  
> 신규 설치는 [INSTALL_UNIFIED.md](INSTALL_UNIFIED.md)를 따르도록 한다.
